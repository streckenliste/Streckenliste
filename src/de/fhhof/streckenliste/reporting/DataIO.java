...
import de.fhhof.streckenliste.reporting.DataFileIO;
 ...
 *
 */
public class DataIO implements DataFileIO {
	/* (non-Javadoc)
	...
	public  Streckenliste readStreckenliste() {
		...
		/*
		 * begin get sollabsch�sse
		 */
		
		
		
		Streckenliste streckenliste=new Streckenliste();
		streckenliste.setSollabschuesse(getSollabschuesse(root.getChild("sollabschuesse")));
		streckenliste.setAmtID(amtID);
		streckenliste.setAnschrUJB(AnschrUJB);
		streckenliste.setRevArt(revart);
		streckenliste.setbSatzart(bSatzart);
		streckenliste.setRevName(revName);
		streckenliste.setRevNr(revNr);
		streckenliste.setVerwert(verwert);
	...
	/*
	 * <jJahr jAbschPlan="2010" nJahre="1">
			<aWildart id="020">
				<aHirschI>14</aHirschI>
				<aHirschIIa>12</aHirschIIa>
				<aHirschIIb>1</aHirschIIb>
				<aHirschIII>22</aHirschIII>
				<aAlttier>5</aAlttier>
				<aSchmaltier>43</aSchmaltier>
				<aKalb>42</aKalb>
	 */
private Sollabschuesse getSollabschuesse(Element soll)
{
	Sollabschuesse sollabsch=new Sollabschuesse();
	List<Element> jJahr=soll.getChildren();
	for(Element aKa:jJahr)
	{
		Sollabschuss sa=new Sollabschuss();
		try
		{
			AKlasse akl=AKlasse.getAKlasseByID((Integer.parseInt(aKa.getAttribute("id").getValue())));
		}
		catch (Exception err)
		{
			//kann Wildart nicht parsen
		}
	}
	return sollabsch;
}
